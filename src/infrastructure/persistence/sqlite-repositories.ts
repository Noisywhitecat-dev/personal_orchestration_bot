...
export function createSqliteRepositories(db: DatabaseSync): Repositories {
  ...
  const s = {
    ...
    insertTask:
      db.prepare(`INSERT INTO tasks (id, project_id, request, state, plan_json, review_round, max_review_rounds, reviews_json,
      codex_session_id, claude_session_id, failure_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`),
    updateTask:
      db.prepare(`UPDATE tasks SET state = ?, plan_json = ?, review_round = ?, max_review_rounds = ?, reviews_json = ?,
      codex_session_id = ?, claude_session_id = ?, failure_json = ?, updated_at = ? WHERE id = ?`),
