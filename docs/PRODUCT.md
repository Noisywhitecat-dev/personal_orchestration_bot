...
브랜드는 AI orchestrator이며 장식형 문구와 소개 카드를 제거한다. 프로젝트 선택은 기존 작업 목록과
마지막 대화를 복원하고, 같은 프로젝트 재선택과 오래된 응답에도 이력을 잃지 않는다.
프리셋은 선택 상태를 표시하고 개별 모델/노력치 편집 시 직접 설정으로 구분한다. 토큰 상한은 보존한다.
메인 사이드바의 계정 사용량은 Codex 주간과 Claude 5시간/주간 비율·초기화 시각을 작업별 토큰과 분리한다.
Codex는 공식 읽기 전용 조회를 제공한다. Claude는 보고된 이벤트 또는 명시적으로 가져온 공식 statusline 데이터만 표시한다.
미보고·초기화 경과·가져온 출처를 구분하고 실시간 또는 비용 추정으로 오인하지 않게 한다.
